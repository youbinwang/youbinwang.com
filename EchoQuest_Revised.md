...
- [2. 战斗系统实现（ComboGraph）](#2-战斗系统实现combograph)
  ...
  - [2.3 动画蒙太奇的配置（AnimNotify）](#23-动画蒙太奇的配置animnotify)
  - [2.4 碰撞检测 ComboGraphCollision](#24-碰撞检测-combographcollision)
  - [2.5 基于动画通知状态的射线检测](#25-基于动画通知状态的射线检测ans-based-trace)
...
### 2.2 以轻攻击 Combo 为例，说明 ComboGraph 在项目中的使用
...
### 2.5 基于动画通知状态的射线检测（ANS-based Trace）
...
### 4.3 以主角 AnimBP 为例，说明 Lyra 架构与程序化动画修正在项目中的实现
...
### 5.3 以近战攻击吸附为例，讲解 Motion Warping 在项目中的实现
...
（插入图[1]：AI 系统文件夹结构总览（Content Browser 中 Content/Enemy 文件夹的展开截图，展示 AI/BehaviorTrees、AI/Tasks、AI/Decorators、AI/EQS、AI/Interfaces 的目录层级））
...
（插入图[2]：BP_Enemy_Base 的 Components 面板截图（在 BP_Enemy_Base 蓝图编辑器中，展示左侧 Components 列表，可见 GAS 相关组件、ComboGraphCollision、WidgetComponent 等核心组件的挂载情况））
...
（插入图[3]：BP_Enemy_Melee 的初始化蓝图逻辑（BP_Enemy_Melee 的 Event Graph → BeginPlay 事件后的初始化流程截图，展示武器挂载、碰撞处理、血条 Widget 创建、掉落物计算的节点链路））
...
（插入图[4]：敌人视觉多样性效果对比（PIE 运行时截图，展示多个近战敌人由于随机材质选择呈现不同外观，或展示敌人死亡溶解效果的过程截图））
...
（插入图[5]：AIC_Enemy_Base 的 AI Perception Component 配置面板（在 AIC_Enemy_Base 蓝图编辑器中，选中 AI Perception Component，展示 Details 面板中 Senses Config 列表，可见 AI Sight、AI Hearing、AI Damage 三种感知配置及其参数））
...
（插入图[6]：On Target Perception Updated 事件回调蓝图（AIC_Enemy_Base 的 Event Graph 中，On Target Perception Updated 事件后的逻辑链路截图，展示感知类型判断的 Switch 分支、黑板变量写入节点和 State 切换逻辑））
...
（插入图[7]：BB_Enemy_Base 黑板变量定义（打开 BB_Enemy_Base 资产，展示 Keys 列表，可见 State、AttackTarget、PointOfInterest、DefendRadius、PatrolRoute 等变量及其类型））
...
（插入图[8]：BT_Enemy_Melee 行为树全览（打开 BT_Enemy_Melee 资产，缩放至能看到完整的顶层 Selector 和四个主分支的程度截图，展示 BeingHit / Combat / Investigating / Passive 四个分支从左到右的排列，每个分支上方可见 Blackboard Decorator 的 State 条件配置））
...
（插入图[9]：SubTree_Frozen 行为子树（打开 BT_SubTree_Frozen 资产，展示内部的完整节点序列：BTT_RotateToPlayer → BTT_ClearFocus → BTT_SetMovementSpeed → Wait））

**分支 2 - Combat State（战斗逻辑序列）：**

这是整个行为树中最复杂的分支，内部进一步拆分为三个子阶段：装备检查、攻击执行和战术走位。

（插入图[10]：Combat State 分支详细结构（在 BT_Enemy_Melee 中，放大 Combat State 分支，展示装备检查、攻击执行和 Strafe 走位三个子 Sequence/Selector 的完整节点树，包括各 Decorator 条件和 Task 叶节点））
...
（插入图[11]：CG_Enemy_Combo 连招资产（打开 CG_Enemy_Combo 资产，展示敌人攻击连招的节点图，包括 Entry 节点、攻击动画蒙太奇节点及右侧的 GE/GC 配置面板））
...
（插入图[12]：SubTree_Investigating 行为子树（打开 BT_SubTree_Investigating 资产，展示 Move to PointOfInterest → Wait → BTT_SetStateAsPassive 的完整节点序列））
...
（插入图[13]：SubTree_Passive 行为子树与巡逻路径（左图：打开 BT_SubTree_Passive 资产，展示 BTD_HasPatrolRoute 装饰器和 BTT_MoveAlongPatrolRoute 节点；右图：关卡编辑器中 BP_PatrolRoute 的 Spline 可视化，展示路径点在场景中的连线效果））

（插入图[14]：PIE 运行时敌人巡逻效果（PIE 运行截图，展示敌人沿巡逻路径移动的实际画面，可配合 AI Debug 显示当前行为树状态））
...
（插入图[15]：BPI_EnemyAI 蓝图接口定义（打开 BPI_EnemyAI 资产，展示接口函数列表，如 SetMovementSpeed、EquipWeapon、ActiveComboGraph 等函数签名及参数定义））

（插入图[16]：BTT 任务节点内部逻辑示例（打开任意一个有代表性的 BTTask，如 BTT_ActiveComboGraph 或 BTT_EquipWeapon，展示其 Event Receive Execute AI 事件后通过 BPI_EnemyAI 接口调用 Pawn 函数的蓝图逻辑））
...
（插入图[17]：EQS_Strafe 查询资产编辑器（打开 EQS_Strafe 资产，展示 Generator（Points: Circle，Center = EQS_Context_AttackTarget，Radius = 400，Points = 5）及下方的 Tests 列表（Distance 和 PathExist）配置））
...
（插入图[18]：EQS 运行时调试可视化（PIE 运行中按下 ' 键启用 AI Debug，选中敌人后切换到 EQS 标签页，展示以玩家为中心的 5 个环形候选点的评分球体：绿色 = 高分可选，红色 = 被 Distance/PathExist 测试剔除，蓝色 = 最终选中的最优点））

（插入图[19]：PIE 运行时敌人战术走位效果（PIE 运行截图，展示敌人在攻击玩家后沿 EQS 查询结果进行环绕走位的实际画面，体现"攻击-走位-再攻击"的战斗循环））
