...
| 190 | 详情页 hero 下方 4 项视觉优化 | ① EN description 去 italic，CN/EN 统一 `font-semibold` 直陈；② Section h2 从 `text-xs muted` → `text-sm secondary` + `pb-3 mb-5 border-b`；③ 标签云三色 legend（Tools 蓝/Skills 橙/Soft 紫 dot + 中英文标签 "工具/设计能力/软实力"），仅显示存在的分类，紧贴 h2 下方；④ 去掉 detail double 上方全宽 hr — section 分隔由两列 h2 underline 担任 |
| 191 | 标签云 spiral 兜底重做 + bullet 圆点 + 主要贡献居中 | **spiral 兜底**：`MIN_HEIGHT 140` 改为按 chip 总面积动态计算 `fitsAllMin = totalChipArea / (PACKING_FACTOR=0.6 × containerW)`，`canvasH = max(MIN_HEIGHT=80, target, fitsAllMin)`；CHIP_GAP `12 → 6` 让 spiral 包更紧。chip 数多 → 自动给更高画布防挤压；chip 少 → 紧贴 anchor。**bullet 圆点**：`·` 字符（U+00B7）在 CN/EN 字体里渲染垂直位置不一致，改用 CSS `<span class="w-1 h-1 mt-[0.55rem] rounded-full bg-text-muted">` 与字体无关。**主要贡献居中**：右栏加 `lg:flex lg:flex-col`、ul 加 `lg:my-auto`，h2 留顶 + bullets 在右栏剩余垂直空间内居中；tag-cloud.ts 新增 `measureAnchorContext()` 在 cloud 比 anchor 自然高度大时给 anchor 设 `min-height = canvasH + usedBefore` 撑开右栏。EN 长内容场景下 my-auto 自动失效无副作用 |
| 192 | Meta 信息区重设计（4 列短分隔） | 移除"每列左 2px 全高竖线"风格；改为每列 dt 上方独立 `w-20 h-px bg-[var(--color-border-strong)] mb-3` 短横线（80px，约 75% 列宽）；移除 dd `min-h-[2lh]` 行高兜底（不再需要）；列间距 `gap-x-3 → gap-x-6`；wrap 在 `<div>` 内，移动端（< sm）改为顶部一条长 hr `sm:hidden`，items 无边框 — 桌面短分隔 + 移动长分隔的响应式适配 |
| 193 | 详情页 hero 下方右栏对齐 | ① EN description 字号 lang-conditional：`lg:text-3xl → lg:text-2xl`（30px → 24px，权重↓约 20%），CN 保持 30px；② 右栏 wrapper 加 `lg:self-stretch`（撑到视频高度），外部链接 div 加 `lg:mt-auto`（自动吸收上方剩余空间钉到底）— description+meta 顶贴 + buttons 与视频底齐平 |
...
| 200 | Phase 2 试点 — the-scholars-side-quest KEY FEATURES 内容迁移完成 | 中英文 MDX 各一份（5 个 h2 节：角色与叙事 / 任务流程 / 场景设计 / 动画制作 / 战斗设计）；从原站 HTML 提取并重命名 29 张 features 图（character/gameplay/map/animation/cutscene/combat 分类）存入 `public/images/games/the-scholars-side-quest/features/`；使用 `<TextImage>` `<ImageRow>` `<ImageGrid>` 三组 layout 组件丰富图文混排；构建验证 71 页零错误 |
...
| 201 | 电影详情页底部导航重构 | 移除 Prev/Next 项目间跳转（三部电影数量少，返回列表更直观）；新增「返回电影列表」/「Back to Films」链接（新增 `film.backToFilms` i18n key，样式与游戏详情页 backToGames 一致）；容器间距 `space-y-16` → `space-y-10`，与游戏详情页统一；清理 `getFilmsByOrder` import 及相关死代码 |
| 203 | CONTEXT.md 游戏图片状态全量更新 | 11 个游戏 hero + gallery 状态表全部标记为 ✅；核实数据来源为 `games.ts` gallery 数组（echo-quest 26 张、elemental-realm 24 张、the-scholars-side-quest 12 张、shepherds 16 张、the-camera 9 张、on-the-road 21 张、aid-master 14 张、baihua-pavilion 12 张、greedy-roots 4 张、elliot-fig 5 张、stars-chat 12 张） |
| 202 | 游戏详情页 KEY FEATURES section header + MDX 字号体系 + 子标题左竖条锚点 | **section header**：取代裸 `<hr>`，统一为 page section 风格 — `<hr>` + `text-2xl md:text-3xl font-bold` h2「核心特性 / Key Features」，与「项目详情」「截图」section 视觉权重一致；新增 `game.keyFeatures` i18n key（zh-cn + en）。**MDX 字号体系**：`GameInlineFeatures.astro` 改用显式 `<style is:global>`（Tailwind v4 prose-h2:md:text-2xl 链式响应式不可靠，已验证不生效）—— h2 `1.25rem md:1.5rem`（**比 page Section h2 小一级**），h3 `1.125rem`，h4 `1rem`；段落 line-height 1.7，列表/段落紧凑节奏。**子标题左竖条锚点**：MDX h2 原下划线方案与 page hr 形成「双横线夹住 page h2」的视觉框格问题；改为 `border-left: 4px solid var(--color-accent) + padding-left: 0.875rem`，page 层级用横线、子层级用橙色竖条，视觉语言完全分离。**CLAUDE.md 字号规范扩展**：新增 MDX h2/h3/h4 三行规则，明确「MDX h2 必须比页面 Section h2 小一级」原因与实现位置 |
| 204 | 全项目 Code Review + P0 修复 | Code review 覆盖数据层 / i18n / 组件 / content collections / 配置 / 死代码六大维度。真实问题 3 个：① `games.ts` Echo Quest + Elemental Realm `coverImage: ''` 占位（cover.png 未生成，列表页/主页 Featured Games 卡片背景空）→ 用户手动补 `public/images/games/{echo-quest,elemental-realm}/cover.png` 后，更新两处 `coverImage` 路径；② `_redirects` 仅覆盖 10 条中文 `/docs/{slug}` 重定向，缺英文 `/en/docs/{slug}` → 补 10 条 `/en/docs/{slug} → /en/games/{slug}/`，避免外链/搜索引擎索引 404；③ 9 个 inline-features MDX 仅 frontmatter（已知 B2 Phase 2 进行中，不算新 bug）。Agent 误报已排除：Stars Chat / On the Road 缺 videoId 是 by design（无视频）；CONTEXT.md Bug #5/#7 状态滞后误判 |
| 205 | Echo Quest cover/hero 图黑边修复 | 原图尺寸 2500×1372（ratio=1.8222），图片本身带有黑色 letterbox 条，`object-cover` 将黑边渲染出来；用户手动裁剪至 2440×1372（ratio=1.7784，≈16:9），直接覆盖原文件，代码路径无需修改；`/games` 列表页视频封面卡 + `/games/echo-quest/` 详情页 Hero 黑边均消除 |
...
| **B2** | **Phase 2 — 10 个项目 MDX 内容迁移（每个项目 1-2 小时）** | ⏳ 进行中 | the-scholars-side-quest ✅ 完成；其余 9 个按内容丰富度排序 |
...
| slug | 状态 |
|---|---|
| echo-quest | hero ✅，gallery ✅（26 张） |
| elemental-realm | hero ✅，gallery ✅（24 张） |
| the-scholars-side-quest | hero ✅，gallery ✅（12 张） |
| shepherds | hero ✅，gallery ✅（16 张） |
| the-camera | hero ✅，gallery ✅（9 张） |
| on-the-road | hero ✅，gallery ✅（21 张） |
| aid-master | hero ✅，gallery ✅（14 张） |
| baihua-pavilion | hero ✅，gallery ✅（12 张） |
| greedy-roots | hero ✅，gallery ✅（4 张） |
| elliot-fig | hero ✅，gallery ✅（5 张） |
| stars-chat | hero ✅，gallery ✅（12 张） |

**全部 11 个游戏 hero + gallery 均已完成。**
